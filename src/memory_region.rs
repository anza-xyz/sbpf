...
pub type MemoryCowCallback = Box<dyn Fn(&mut MemoryRegion, AccessType, u64, u64)>;
/// Fail always
...
macro_rules! access_violation_guard {
    ($self:expr, $region:expr, ($access_type:expr, $vm_addr:expr, $len:expr), $host_addr:ident, $result:expr) => {{
        ...
        // which in turn guarantees that no two mutable references to $region exist simultaneously.
        (&$self.cow_cb.borrow_mut())(
            unsafe {
                // Same as: &mut *(&raw const *$region).cast_mut()
                &mut *(*($region as *const _ as *const UnsafeCell<MemoryRegion>)).get()
            },
            $access_type,
            $vm_addr,
            $len,
...
impl<'a> UnalignedMemoryMapping<'a> {
    ...
    pub fn map(&self, access_type: AccessType, vm_addr: u64, len: u64) -> ProgramResult {
        if let Some((_index, region)) = self.find_region(vm_addr) {
            access_violation_guard!(
                self,
                region,
                (access_type, vm_addr, len),
                host_addr,
