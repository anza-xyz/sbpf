    ...
    fn verify<C: ContextObject>(prog: &[u8], _config: &Config, sbpf_version: SBPFVersion, _function_registry: &FunctionRegistry<usize>, syscall_registry: &FunctionRegistry<BuiltinFunction<C>>) -> Result<(), VerifierError> {
        ...
        while (insn_ptr + 1) * ebpf::INSN_SIZE <= prog.len() {
            ...
            match insn.opc {
                ...
                // BPF_JMP32 class
                ebpf::JEQ32_IMM  if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JEQ32_REG  if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JGT32_IMM  if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JGT32_REG  if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JGE32_IMM  if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JGE32_REG  if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JLT32_IMM  if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JLT32_REG  if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JLE32_IMM  if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JLE32_REG  if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JSET32_IMM if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JSET32_REG if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JNE32_IMM  if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JNE32_REG  if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JSGT32_IMM if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JSGT32_REG if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JSGE32_IMM if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JSGE32_REG if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JSLT32_IMM if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JSLT32_REG if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JSLE32_IMM if sbpf_version.enable_jmp32() => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ...
                ebpf::JEQ64_IMM    => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JEQ64_REG    => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JGT64_IMM    => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JGT64_REG    => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JGE64_IMM    => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JGE64_REG    => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JLT64_IMM    => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JLT64_REG    => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JLE64_IMM    => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JLE64_REG    => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JSET64_IMM   => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JSET64_REG   => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JNE64_IMM    => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JNE64_REG    => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JSGT64_IMM   => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JSGT64_REG   => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JSGE64_IMM   => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JSGE64_REG   => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JSLT64_IMM   => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JSLT64_REG   => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
                ebpf::JSLE64_IMM   => { check_jmp_offset(prog, insn_ptr, &function_range)?; },
