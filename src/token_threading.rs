...
#[cfg(not(test))]
use crate::{memory_region::MemoryMapping, static_analysis::RegisterTraceEntry, vm::RuntimeEnvironmentSlot};
use {
    crate::{elf::Executable, error::{EbpfError, ProgramResult}, program::BuiltinProgram, static_analysis::DummyContextObject, vm::{ContextObject, EbpfVm}},
    rand::{distributions::{Distribution, Uniform}, rngs::SmallRng, SeedableRng},
    std::sync::Arc,
};
...
const DECODE_IMM32_PATTERN: [u8; 4] = [0x4C, 0x63, 0x58, 0x04]; // movsxd r11, dword ptr [rax + 4]
...
pub fn compile<C: ContextObject>(executable: &Executable<C>) -> Result<(Vec::<u32>, Vec::<u8>), EbpfError> {
    ...
    let program: &[u64] = unsafe { std::slice::from_raw_parts(sbpf_text_section.as_ptr().cast(), sbpf_text_section.len() / 8) };
    let instruction_templates = unsafe { std::slice::from_raw_parts(INSTRUCTION_TEMPLATES as *const [u8; 64], 65536) };
    ...
    while let Some((program_counter, insn)) = iter.next() {
        ...
        if instruction_template_meta & INSN_HAS_IMM != 0 {
            if *insn as u8 == crate::ebpf::LD_DW_IMM {
                let Some((_, insn2)) = iter.next() else { return Err(EbpfError::InvalidInstruction); };
                let value = ((insn >> 32) as u32).wrapping_sub(immediate_value_key as u32) as u64 | (((insn2 >> 32) as u32).wrapping_sub((immediate_value_key >> 32) as u32) as u64) << 32;
                unsafe {
                    text_section.as_mut_ptr().add(text_section.len() - instruction_template.len() + 2).cast::<u64>().write_unaligned(value);
                    text_section.as_mut_ptr().add(text_section.len() - instruction_template.len() + 12).cast::<u64>().write_unaligned(immediate_value_key as u64);
                }
            } else {
                let value = ((insn >> 32) as u32).wrapping_sub(immediate_value_key as u32);
                unsafe {
                    text_section.as_mut_ptr().add(text_section.len() - instruction_template.len() + 3).cast::<u32>().write_unaligned(value);
                    text_section.as_mut_ptr().add(text_section.len() - instruction_template.len() + 10).cast::<u32>().write_unaligned(immediate_value_key as u32);
                }
            }
        }
        match opcode_class {
            crate::ebpf::BPF_LD | crate::ebpf::BPF_LDX | crate::ebpf::BPF_ST | crate::ebpf::BPF_STX => unsafe {
                text_section.as_mut_ptr().add(text_section.len() - instruction_template.len() + 3).cast::<u16>().write_unaligned((insn >> 16) as u16);
            },
            crate::ebpf::BPF_JMP32 | crate::ebpf::BPF_JMP64 => {
                let target_program_counter = program_counter as i32 + ((insn >> 16) as i16 as i32);
                unsafe {
                    text_section.as_mut_ptr().add(text_section.len() - instruction_template.len()).cast::<u32>().write_unaligned(program_counter as u32);
                }
                if instruction_template_index == (crate::ebpf::CALL_IMM as usize | (1 << 12)) {
                    let fn_ptr = match syscall_resolver(loader, (insn >> 32) as u32) {
                        ProgramResult::Ok(fn_ptr) => fn_ptr,
                        ProgramResult::Err(err) => return Err(err),
                    };
                    unsafe {
                        text_section.as_mut_ptr().add(text_section.len() - 8).cast::<u64>().write_unaligned(fn_ptr);
                    }
                } else {
                    let jmp_offset = pc_section[target_program_counter as usize] as i32 - pc_section[program_counter] as i32;
                    unsafe {
                        text_section.as_mut_ptr().add(text_section.len() - instruction_template.len() + 7).cast::<u32>().write_unaligned(target_program_counter as u32);
                        text_section.as_mut_ptr().add(text_section.len() - 4).cast::<i32>().write_unaligned(jmp_offset);
                    }
                }
            },
            _ => {},
        ...
        if next_noop_insertion == 0 {
            // TODO: Bench against deriving this from difference in text_section instead of sampling the rng again
            next_noop_insertion = noop_range.sample(&mut diversification_rng_second);
            text_section.push(0x90);
        } else {
            next_noop_insertion -= 1;
        }
    }
